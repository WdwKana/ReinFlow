...
from model.flow.mlp_flow import FlowMLP, VisionFlowMLP, NoisyFlowMLP
...
class Args:
    ...
    # ReinFlow specific arguments
    use_reinflow: bool = False
    """if toggled, use ReinFlow agent instead of vanilla PPO"""
    pretrained_model_path: Optional[str] = None
    """path to pretrained ReinFlow model checkpoint"""
    horizon_steps: int = 4
    """action sequence length for ReinFlow"""
    inference_steps: int = 1
    """number of denoising steps for inference"""
    ft_denoising_steps: int = 1
    """number of denoising steps for fine-tuning"""
    min_sampling_denoising_std: float = 0.08
    """minimum std for sampling during denoising"""
    min_logprob_denoising_std: float = 0.08
    """minimum std for logprob calculation during denoising"""
    max_logprob_denoising_std: float = 0.14
    """maximum std for logprob calculation during denoising"""
    clip_ploss_coef: float = 0.01
    """policy loss clipping coefficient for ReinFlow"""
    denoised_clip_value: float = 1.0
    """clipping value for intermediate denoised actions"""
    time_dim_explore: int = 0
    """time embedding dimension for exploration noise"""
    noise_hidden_dims: list = None
    """hidden dimensions for noise network"""
    use_bc_loss: bool = False
    """if toggled, use behavior cloning loss"""
    bc_loss_type: str = 'W2'
    """type of behavior cloning loss"""
    normalize_denoising_horizon: bool = False
    """if toggled, normalize denoising horizon in loss calculation"""
    normalize_act_space_dimension: bool = False
    """if toggled, normalize action space dimension in loss calculation"""
    clip_intermediate_actions: bool = True
    """if toggled, clip intermediate actions during denoising"""
    account_for_initial_stochasticity: bool = True
    """if toggled, account for initial stochasticity in loss calculation"""
...
class ReinFlowDictArray(DictArray):
    """
    扩展原有的DictArray以支持动作序列存储
    """
    def __init__(self, buffer_shape, element_space, data_dict=None, device=None, horizon_steps=4):
        super().__init__(buffer_shape, element_space, data_dict, device)
        self.horizon_steps = horizon_steps
        
        # 添加动作序列存储（如果还没有的话）
        if data_dict is None and 'action_sequences' not in self.data:
            # 假设action space是Box类型
            action_shape = None
            for k, v in element_space.items():
                if 'action' in k.lower() or k == 'actions':
                    action_shape = v.shape
                    break
            
            if action_shape is not None:
                seq_shape = buffer_shape + (horizon_steps,) + action_shape
                self.data['action_sequences'] = torch.zeros(seq_shape).to(device)
...
class ReinFlowAgent(nn.Module):
    """
    ReinFlow Agent that wraps PPOFlow to be compatible with the original Agent interface
    """
    def __init__(self, envs, sample_obs, args):
        ...
        self.horizon_steps = args.horizon_steps
        self.action_dim = envs.single_action_space.shape[0]
        
        # 初始化特征提取器（复用原来的NatureCNN）
        self.feature_net = NatureCNN(sample_obs=sample_obs)
        latent_size = self.feature_net.out_features
        
        # 使用ViT Critic来匹配预训练模型
        self.critic = self._create_vit_critic(args)
        
        # 初始化PPOFlow（简化版本）
        self._init_ppoflow(latent_size, envs)
    ...
    def _create_vit_critic(self, args):
        ...
        try:
            # 检查是否有rgb输入
            if not args.include_rgb:
                # 如果没有rgb，使用简单的MLP critic
            ...
            # 使用ViT Critic（简化版本，需要根据实际obs_shape调整）
            critic = ViTCritic(
                spatial_emb=128,
                augment=False,
                backbone=VitEncoder(
                    obs_shape=[3, 128, 128],  # 需要根据实际图像尺寸调整
                    num_channel=3,
                    img_h=128,
                    img_w=128,
                    cfg={
                ...
                residual_style=True,
                cond_dim=25  # 需要根据实际state维度调整
            )
            return critic
        except Exception as e:
            print(f"Warning: Failed to create ViT Critic ({e}), falling back to MLP critic")
            # 回退到简单的MLP critic
            return nn.Sequential(
                layer_init(nn.Linear(self.feature_net.out_features, 512)),
                nn.ReLU(inplace=True),
    ...
    def _init_ppoflow(self, obs_dim, envs):
        """初始化PPOFlow模型"""
        # 创建简化的Flow模型
        policy = FlowMLP(
            horizon_steps=self.horizon_steps,
            action_dim=self.action_dim,
            cond_dim=obs_dim,  # 使用特征维度
            time_dim=32,
        ...
        # 设置默认参数
        if self.args.noise_hidden_dims is None:
            self.args.noise_hidden_dims = [256, 256, 256]
        ...
        self.ppoflow = PPOFlow(
            ...
            policy=policy,
            critic=self.critic,
            actor_policy_path=self.args.pretrained_model_path,
            act_dim=self.action_dim,
            horizon_steps=self.horizon_steps,
            act_min=float(envs.single_action_space.low[0]),
            act_max=float(envs.single_action_space.high[0]),
            obs_dim=obs_dim,
            cond_steps=1,
            noise_scheduler_type='learn',
            inference_steps=self.args.inference_steps,
            ft_denoising_steps=self.args.ft_denoising_steps,
            randn_clip_value=3.0,
            min_sampling_denoising_std=self.args.min_sampling_denoising_std,
            min_logprob_denoising_std=self.args.min_logprob_denoising_std,
            max_logprob_denoising_std=self.args.max_logprob_denoising_std,
            logprob_min=-1.0,
            logprob_max=1.0,
            clip_ploss_coef=self.args.clip_ploss_coef,
            clip_ploss_coef_base=0.001,
            clip_ploss_coef_rate=3,
            clip_vloss_coef=None,
            denoised_clip_value=self.args.denoised_clip_value,
            time_dim_explore=self.args.time_dim_explore,
            learn_explore_time_embedding=False,
            use_time_independent_noise=True,
            noise_hidden_dims=self.args.noise_hidden_dims,
            logprob_debug_sample=False,
            logprob_debug_recalculate=False,
            explore_net_activation_type='Tanh'
        )
        
        print(f"Initialized ReinFlow with horizon_steps={self.horizon_steps}, inference_steps={self.args.inference_steps}")
    ...
    def get_action_and_value(self, x, action=None):
        """获取动作和价值（用于训练）"""
        # 处理observation的格式，使其与PPOFlow兼容
        if isinstance(x, dict):
            # 如果输入已经是字典格式（包含rgb等）
            cond = x
            if 'state' in cond and len(cond['state'].shape) == 2:
                # 添加时间维度 [B, D] -> [B, 1, D]
                cond = {k: v.unsqueeze(1) if k == 'state' and len(v.shape) == 2 else v 
                       for k, v in cond.items()}
        else:
            # 向后兼容：如果输入是tensor，则只使用特征
            features = self.feature_net(x) if not hasattr(x, 'keys') else x
            cond = {"state": features.unsqueeze(1) if len(features.shape) == 2 else features}
        
        # 获取value（根据critic类型选择不同的调用方式）
        if hasattr(self.critic, 'forward') and 'no_augment' in str(self.critic.forward):
            # ViT Critic
            value = self.critic(cond, no_augment=True).flatten()
        else:
            # MLP Critic  
            if isinstance(cond, dict) and 'state' in cond:
                features = cond['state'].squeeze(1)  # 移除时间维度用于MLP
            else:
                features = self.feature_net(x)
        ...
        if action is None:
            # 生成新的动作序列和chains
            action_seq, chains, logprob = self.ppoflow.get_actions(
                cond=cond,
                eval_mode=False,
                save_chains=True,  # 保存chains用于训练
                ret_logprob=True
            )
            ...
            # 存储序列和chains用于后续步骤和训练
            self.current_action_sequence = action_seq
            self.training_chains = chains
            self.action_step = 1
            
            # 计算熵（这里仍然简化，完整版本应该从PPOFlow获取）
            entropy = torch.ones_like(logprob) * 0.1
            
        else:
            # 训练时重新计算log概率（不会被调用，因为训练时使用PPOFlow.loss）
            logprob = torch.zeros(action.shape[0], device=action.device)
            entropy = torch.ones_like(logprob) * 0.1
        
        return action, logprob, entropy, value
    
    def _generate_new_action_sequence(self, x, deterministic=False):
        """生成新的动作序列"""
        features = self.feature_net(x)
        cond = {"state": features.unsqueeze(1)}
        
        action_seq = self.ppoflow.get_actions(
            cond=cond,
...
if __name__ == "__main__":
    ...
    # ReinFlow specific: store chains for training
    if args.use_reinflow:
        # Get action_dim from environment
        action_dim = envs.single_action_space.shape[0]
        # chains shape: (num_steps, num_envs, inference_steps+1, horizon_steps, action_dim)
        chains = torch.zeros((args.num_steps, args.num_envs, args.inference_steps+1, args.horizon_steps, action_dim)).to(device)
    else:
        chains = None
    ...
    if args.use_reinflow:
        print("Using ReinFlow Agent")
        # Set device for args
        args.device = device
        agent = ReinFlowAgent(envs, sample_obs=next_obs, args=args).to(device)
    elif MODE not in ['state', 'state_oracle']:
        print("Using standard Agent")
        agent = Agent(envs, sample_obs=next_obs).to(device)
    else:
        print("Using AgentStateOnly")
        agent = AgentStateOnly(envs).to(device)

    # Initialize optimizers
    if args.use_reinflow:
        # Separate optimizers for actor and critic in ReinFlow
        actor_optimizer = optim.Adam(agent.ppoflow.actor_ft.parameters(), lr=args.learning_rate, eps=1e-5)
        critic_optimizer = optim.Adam(agent.ppoflow.critic.parameters(), lr=args.learning_rate, eps=1e-5)
        optimizer = None  # We'll use separate optimizers for ReinFlow
    else:
    ...
    for iteration in tqdm(range(1, args.num_iterations + 1), total=args.num_iterations, desc="Training"):
        ...
        for step in range(0, args.num_steps):
            ...
            # Store chains for ReinFlow training
            if args.use_reinflow and hasattr(agent, 'training_chains') and agent.training_chains is not None:
                chains[step] = agent.training_chains.detach()
        ...
        # ReinFlow specific: flatten chains
        if args.use_reinflow:
            action_dim = envs.single_action_space.shape[0]
            b_chains = chains.reshape((-1, args.inference_steps+1, args.horizon_steps, action_dim))
        else:
            b_chains = None
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.use_reinflow:
                    ...
                    noise_std, newQ_values = agent.ppoflow.loss(
                        ...
                        normalize_act_space_dimension=args.normalize_act_space_dimension,
                        verbose=False,  # 训练时减少输出
                        clip_intermediate_actions=args.clip_intermediate_actions,
                        account_for_initial_stochasticity=args.account_for_initial_stochasticity
                    )
                    
                    # 计算entropy（从loss返回的是负entropy）
                    ...
                    # 总损失
                    loss = pg_loss + entropy_loss * args.ent_coef + v_loss * args.vf_coef + bc_loss * 0.0  # bc_loss权重
                    ...
                    # 梯度裁剪
                    nn.utils.clip_grad_norm_(agent.ppoflow.actor_ft.parameters(), args.max_grad_norm)
                    nn.utils.clip_grad_norm_(agent.ppoflow.critic.parameters(), args.max_grad_norm)
                    ...
                    # 更新变量以保持兼容性
                    newvalue = torch.tensor([newQ_values] * len(mb_inds)).to(device)
                    newlogprob = torch.tensor([newlogprob_min] * len(mb_inds)).to(device)  # 简化
        ...
        if args.use_reinflow:
            logger.add_scalar("charts/actor_learning_rate", actor_optimizer.param_groups[0]["lr"], global_step)
            logger.add_scalar("charts/critic_learning_rate", critic_optimizer.param_groups[0]["lr"], global_step)
        else:
